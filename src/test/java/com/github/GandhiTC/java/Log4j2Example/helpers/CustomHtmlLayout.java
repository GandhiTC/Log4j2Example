package com.github.GandhiTC.java.Log4j2Example.helpers;
 ...
 * <p>
 * Appenders using this layout should have their encoding set to UTF-8 or UTF-16, otherwise events containing non ASCII characters
 * could result in corrupted log files.
 * </p>
 */
...
//		sbuf = removeString(sbuf, "</body></html>");
//		sbuf = removeString(sbuf, "<br>");
//		sbuf = removeString(sbuf, "</table>");
		
		sbuf.append(Strings.LINE_SEPARATOR).append("<tr>").append(Strings.LINE_SEPARATOR);
		sbuf.append("<td>");
//        sbuf.append(event.getTimeMillis() - jvmStartTime);
		SimpleDateFormat	formatter		= new SimpleDateFormat("MMM-dd-yyyy  hh:mm:ss.SSS a zzz (Z)");
		String				formattedDate	= formatter.format(new java.util.Date(event.getTimeMillis()));
		sbuf.append(formattedDate);
...
//		sbuf.append("<td title=\"").append(escapedThread).append(" thread\">");
		final String escapedThread = Transform.escapeHtmlTags(String.format("%04d", event.getThreadId()));
		sbuf.append("<td title=\"Thread ID ").append(escapedThread).append("\">");
	...
	private StringBuilder removeString(final StringBuilder sbuilder, final String s)
	{
		int ef = sbuilder.indexOf(s);
		
		if(ef != -1)
		{
			sbuilder.delete(ef, ef + s.length());
			removeString(sbuilder, s);
		}
...
//		appendLs(sbuf, "Log session start time " + formattedDate + "<br>");
...
//		sbuf.append("<tr><td bgcolor=\"#D3D3D3\" colspan=\"6\">&nbsp;</td></tr>");
		sbuf.append("<tr><td bgcolor=\"#D3D3D3\" colspan=\"6\"></td></tr>");
	 ...
	 * 
	 * @param locationInfo
	 *                     If "true", location information will be included. The default is false.
	 * @param title
	...
	public static class Builder implements org.apache.logging.log4j.core.util.Builder<CustomHtmlLayout>
	{
		...
		// defer default value in order to use specified charset
		@PluginBuilderAttribute
		...
		public CustomHtmlLayout build()
		{
			...
			return new CustomHtmlLayout(locationInfo, title, contentType, charset, fontName, fontSize.getFontSize(), fontSize.larger().getFontSize());
		}
	}
}